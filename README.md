...
- **Turning maze** (`/turning/`): a maze on an object that turns, in any of three mechanisms — a stack of rings threaded on a dowel, two identical solids glued at a face and free to turn against each other, or one solid cut open along its own edges into three pieces that turn against one another (five solids offer such a cut; turning one renames it, so a rhombicuboctahedron becomes J37 and a rhombicosidodecahedron J72 and J73). Every way of turning it is a different maze, and the design is searched so that *every one of them is a perfect maze*. Drag a piece to turn it, watch the solution re-route each time it clicks into place, and export the printable pieces as PDF

- **Folding maze** (`/fold/`): a maze on cubes taped into a ring, in any of four objects — eight cubes (the fidget toy that folds forever), ten that shut into a **frame with a hole through it**, which is the fewest that can (eight provably cannot), and two rings of twelve, one folding between a frame, a plank and a solid block and the other between five shapes. Where a shape has a hole in it the same printed pattern has to be a perfect maze on a torus as well as on a sphere. Folded shut, cubes press together and part of the maze goes *inside the object*, so each shape can show a different number of cells — half as much again in one shape as in another, on the twelve-cube frame; one printed design is a perfect maze in every one of them. Left alone it folds from shape to shape by itself, every shape once a round, and asking for one sends it there — an arc at a time, each fold swung the way round that misses the rest of the object; most pairs of shapes are more than one fold apart, so it goes through shapes it cannot be put down in. A maze is named by an object and a seed, as the polyhedral page names one by a solid and a seed, and found in about a second at any ruling on offer — two to ten squares across a face; the first few seeds at each ruling of each object ship with the page so that opening it never waits. The entrance and the exit are each printed on *two* squares, since a ring of cubes buries part of itself and what it buries changes: exactly one of each pair is outside whatever you fold it into

## Demo

- Polyhedra: [https://kakeami.github.io/polyhedral-maze/](https://kakeami.github.io/polyhedral-maze/)
- Turning maze: [https://kakeami.github.io/polyhedral-maze/turning/](https://kakeami.github.io/polyhedral-maze/turning/)
