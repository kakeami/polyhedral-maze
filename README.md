...
- **Folding maze** (`/fold/`): a maze on eight cubes taped into a ring — the fidget toy that folds forever. Folded shut, two cubes press together and half the maze goes *inside the object*, so each of its six shapes shows a different number of cells; one printed design is a perfect maze in all six. Left alone it folds from shape to shape by itself, and asking for one sends it there — an arc at a time, each fold swung the way round that misses the rest of the object; most pairs of shapes are more than one fold apart, so it goes through shapes it cannot be put down in. A maze is named by a seed, as on the polyhedral page, and found in about a second at any ruling on offer — two to ten squares across a face; the first few seeds at each ruling ship with the page so that opening it never waits. The entrance and the exit are each printed on *two* squares, since no square of a ring of cubes is on show in every pose: exactly one of each pair is outside whatever you fold it into
