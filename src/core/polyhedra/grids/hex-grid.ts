...
/**
 * HexGrid: 6-sector triangular grid for hexagonal faces.
 *
 * Each hexagon is divided into 6 sectors from the face center to
 * consecutive vertex pairs. Each sector uses a TriGrid-like layout
 * with n rows and n² cells. Total: 6n² cells per face.
 *
 * Cell addressing: row = sector * n + localRow, col = localCol.
 */
export class HexGrid implements FaceGrid {
  readonly faceId: number;
  readonly n: number;
  readonly kind: GridKind = 'hex';
  ...
  constructor(face: Face, n: number) {
    ...
    for (let s = 0; s < 6; s++) {
      ...
      this._sectorV.push(sub(face.vertices[(s + 1) % 6]!, this._center));
    }
